...
async def signup_post(
    ...
    db.commit()
    regenerate_session(request, user_id=user.id)
    messages.success(request, f"First Welcome, {username}")
    return RedirectResponse("/users/profile", status_code=302)
...
async def profile(
    ...
    user = request.state.user
    msg = messages.pop_all(request)
    return templates.TemplateResponse("profile.html", {"request": request, "user": user, "messages":msg})
```
...
.flash {
  ...
  animation: fadeIn 0.3s ease, fadeOut 0.3s ease 4.5s forwards;
}
...
@keyframes fadeOut {
  ...
  <div class="flash-container">
    {% for m in messages %}
      <div class="flash flash-{{ m.level }} {{ m.tags|default('') }}">{{ m.message }}</div>
    {% endfor %}
  </div>
  {% endif %}
